...
ref(1:2:end) = r1;
ref(2:2:end) = r2;
% ref = zeros(length(ref),1);
% ref(1:2:end) = -hs(1);
% ref(2:2:end) = -hs(2);

% Create an arbitrary cost
C = eco_cost(1200,length(ref));
...
yd = []; zd = []; Td = []; ud = []; set_p = []; x = [0;0;0;0]; u = [0;0]; d = [0;0];
xh = [0;0;0;0]; Pk = 10; xhd =[]; Pd = []; Qk = 100*eye(2); Rk = 10*eye(2);
for k = sim_length
    % Simulation
    d = chol(Qk)'*randn(2,1);
    x = Ad*x+Bd*u+Bvd*d;
    ...
    % State Estimation
    [xh,Pk] = statestimator(xh,Pk,Ad,Bd,Bvd,Cd,u,z,Rk,Qk);
    xhd = [xhd xh];
    
    % Select reference
    i_ref = 2*k+1;
    R = ref(i_ref:(i_ref+2*n-1));
    R_bar = R - Phi*xh;% - Gamma_d*d d = 0 for deviation
    gu = C(i_ref:(i_ref+2*n-1));
    ...
    b_u = [-U_min;U_max;zeros(2*n,1);-D_U_min;D_U_max;-R_bar];
...
sgtitle('Economic Linear MPC','FontSize',11)
...
figure('Units','inches','Position',[0 0 6.693 2.5],'PaperPositionMode','auto');
set(0, 'defaultAxesTickLabelInterpreter','latex'); set(0, 'defaultLegendInterpreter','latex');
set(0,'defaultTextInterpreter','latex');

subplot(1,1,1)
plot(k_0:Ts:k_f*Ts,C(1:2:k_f*2+2),'k','linewidth',1.1)
hold on
plot(k_0:Ts:k_f*Ts,C(2:2:k_f*2+2),'Color', [0.5 0.5 0.5],'linewidth',1)
grid('on')
xlim([k_0 k_f*Ts])
ylabel('$c$')
xlabel('Time [s]')
legend('$c_{u_1}$','$c_{u_2}$','Location','East')

sgtitle('Cost over simulation period','FontSize',11)
set(subplot(1,1,1), 'Position', [.065, .15, .90, .72]);