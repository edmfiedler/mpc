...
Qk = 0.1*eye(2); Rk = 1*eye(2);
% SDE matrices
As = [Ad Bvd; zeros(2,4) eye(2)];
Bs = [Bd;zeros(2,2)];
Bvs = [zeros(4,2); Qk];
Cs = [C zeros(2,2)];

% Initial conditions
zd = []; yd = []; Td = []; ud = []; set_p = []; x = [0;0;0;0;0;0]; u = [0;0]; d = [0;0];
xh = [0;0;0;0]; Pk = 10; xhd =[]; Pd = [];
for k = sim_length
    % Simulation
    d = chol(5*eye(2))*d_rand(:,k+1);
    x = As*x+Bs*u+Bvs*d;
...
sgtitle('Input-constrained and Soft-output-constrained MPC controlling linearised SDE model','FontSize',11)
